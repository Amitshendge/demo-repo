...
# Let's say, Apple. (NO numpy, pandas or anything fancy here in this homework.)
# 
...
#   :param firstdate: (python list) the first date (end of list) of the price list
#   :param lastdate: (python list) the last date (beginning of list) of the price list
...
#   :self.dates: = [] # (python list) starts from the latest/newest date, for easy access of newer data when the list gets long
#   :self.price_eod: = [] # (python list) import from data file
#   :self.volumes: = [] # (python list) import from data file
#   :self.delta1: = [] # (python list) daily change values, or the first derivative, that is the previous close minus today's close, example price_eod[0] - price_eod[1], price_eod[1] - price_eod[2]
#   :self.delta2: = [] # (python list) the second derivative, calculate from teh first derivative. Basicallly, the daily change in the first derivative (over 1 day), is the second derivative. 
# 
# Methods:
...
class Stock:
  ...
  def import_history(self, filepath):
    ...
    # fh.close() # close the file handle w
  ...
  def insert_newday(self, newdate, newprice, newvolume):
    ...
    # calculate and insert new data to delta1
    self.delta1.insert(0,round(float(self.price_eod[0])-float(self.price_eod[1]),2))
    # calculate and insert new data to delta2
...
# 
print(aapl.nday_change_percent(50),aapl.nday_change_percent(200),aapl.nday_change_percent(600))
print(msft.nday_change_percent(50),msft.nday_change_percent(200),msft.nday_change_percent(600))
print(goog.nday_change_percent(50),goog.nday_change_percent(200),goog.nday_change_percent(600))

#Google performed the best in 50 days,MSFT performed better in 200 days and 600 days.
