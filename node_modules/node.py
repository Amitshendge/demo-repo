#%%[markdown]
# # HW - Classifiers on Digits dataset
# 
...
xlabels = [ 'x'+('00'+str(i))[-3:] for i in range(785)] # 785 data columns
# 'x000 (first column is y target label, value 0-9), x001, x002, etc there are 28x28=784 pixels columns, 
dfdigits = pd.read_csv(filename, names=xlabels) 

# consider using just a smaller subset with fewer rows. There are 20k rows here. Maybe use 8k or something like that for below. 
# Since the rows itself are random (in terms of the target y-value), it is okay to just pick, say the first 8k rows here. 
# We can call the smaller subset 'smdigits' for example.
smdigits = dfdigits.iloc[:8000,]
print("Shape : ",smdigits.shape)
print("Data Type: ", smdigits.dtypes)
print("\nReady to continue.")
...
x = smdigits.drop(["x000"],axis = 1)
y = smdigits["x000"]

x_train,x_test,y_train,y_test = train_test_split(x,y,test_size=0.2, random_state= 268)
...
plt.gray() 
plt.matshow(x_train.iloc[-1,:].values.reshape(28,28)) 
plt.show() 
...
logisticRegression_fit = logisticRegression.fit(x_train, y_train)
print(f'logistic regression train score: {logisticRegression_fit.score(x_train,y_train)}')
print(f'logistic regression test score: {logisticRegression_fit.score(x_test,y_test)}')
print(confusion_matrix(y_test, logisticRegression_fit.predict(x_test)))
print(classification_report(y_test, logisticRegression_fit.predict(x_test)))
...
print(f'svc train score: {knn.score(x_train,y_train)}')
print(f'svc test score: {knn.score(x_test,y_test)}')
print(confusion_matrix(y_test, knn.predict(x_test)))
print(classification_report(y_test, knn.predict(x_test)))

#When K= 7 it give highest accuracy
...
for criterion in crit:
    ...
    decisionTree = DecisionTreeClassifier(max_depth=maxlevel, criterion=criterion, random_state=1)
    decisionTree.fit(x_train,y_train)
    print(f'Decision Tree train score:  {decisionTree.score(x_train,y_train)}')
    print(f'Decision Tree test score:  {decisionTree.score(x_test,y_test)}')
    print(confusion_matrix(y_test, decisionTree.predict(x_test)))
    print(classification_report(y_test, decisionTree.predict(x_test)))

maxlevel=10
crit = 'entropy' 
decisionTree = DecisionTreeClassifier(max_depth=maxlevel, criterion=crit, random_state=1)
decisionTree.fit(x_train,y_train)
print(f'Decision Tree train score: {decisionTree.score(x_train,y_train)}')
print(f'Decision Tree test score: {decisionTree.score(x_test,y_test)}')
print(confusion_matrix(y_test, decisionTree.predict(x_test)))
...
# Decision tree gives the best results when criterion = Entropy.
#%%
...
%timeit print(f'\ntree CV accuracy score: { cross_val_score(decisionTree, X_cv, y_cv, cv= 10, scoring="accuracy", n_jobs = -1) }\n')   

%timeit print(f'\nknn CV accuracy score: { cross_val_score(knn, X_cv, y_cv, cv= 10, scoring="accuracy", n_jobs = -1) }\n')   

%timeit -r 1 print(f'\nlsvc CV accuracy score: { cross_val_score(linearsvc, X_cv, y_cv, cv= 10, scoring="accuracy") }\n')   

%timeit -r 1 print(f'\nsvcl CV accuracy score: { cross_val_score(svcKernelLinear, X_cv, y_cv, cv= 10, scoring="accuracy", n_jobs = -1) }\n')   

%timeit -r 1 print(f'\nSVC CV accuracy score: { cross_val_score(svc, X_cv, y_cv, cv= 10, scoring="accuracy", n_jobs = -1) }\n')   
# %%
# for nmax = 4000
nmax = 4000 
cvdigits = dfdigits.iloc[0:nmax,:]
