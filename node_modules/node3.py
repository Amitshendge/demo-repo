...
while year<2025:
  for i in range(8,21):
    print(monthofyear[i%12], year)
    if i%12==11:
      year+=1
    if year ==2025 and i%12==4:
      break
...
def to_gradepoint(grade):
  ...
  # gradepoint = ???
  gradepoint = None
  inputs = ['A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+','D','F']
  if grade not in inputs:
    print("Enter the valid gpa")
  else:
    if grade =='A':
      gradepoint = 4.0
    elif grade == "A-":
...
def to_gradepoint_credit(course):
  ...
  grade_point = to_gradepoint(course["grade"])
  grade_point_credit = grade_point*course["credits"]
...
def find_gpa(courses):
  ...
  for c in courses:
      grade_point = to_gradepoint_credit(c)
      credit = c["credits"]
      total_grade_point_credit += grade_point
...
def printCourseRecord(course):
  ...
  # ??????    fill in your codes here
    ...
    return f'{course["year"]} {course["semester"]} - {course["id"]} : {course["class"]} ({course["credits"]}) {course["grade"]} Grade point credits: {"%.2f"%to_gradepoint_credit(course)}'                                        # or return None

  
  
# Try:
print(printCourseRecord(course))
...
def printTranscript(courses):
  ...
  print("Cumulative GPA: {gpa}".format(gpa=gpa))
